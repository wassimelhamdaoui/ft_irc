...
std::string server::part1(std::vector<std::string> tokens, Client &client)
{
    std::string response = "";
...
std::string server::part_response(std::vector<std::string> tokens, Client &client)
{
    ...
    if (tokens[0] == "PART" && tokens.size() > 0)
    {
        ...
        else if (tokens.size() == 2)
        {
            ...
            //     {
            //         if (this->_channels.count(channels[i]) > 0 && this->romoved_channel == false)
            //         {
            ...
            //             {
            //                 //this->_channels.erase(channels[i]);
            //                 this->romoved_channel = true;
            //             }
           ...
           if (!has_comma(tokens[1]))
            {
                response = this->part1(tokens, client);
            }

...
/*   Updated: 2023/04/16 14:47:39 by mabdelba         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
std::string 	server::kick_response(std::vector<std::string> split, Client &client)
{
	...
	if(!client.get_print())
		return (":localhost 451 * KICK :You must finish connecting with nickname first.\r\n");
	if(split.size() < 3)
	...
	if(split.size() > 3 && split[3][0] == ':' && split[3].length() > 1)
	{
		flg = 1;
	...
	channel.broadcast_message(":" + client.get_nick() + "!~" + client.get_userName() + "@localhost KICK " + User.get_nick() + " out of " + channel.get_name() + msg +"\r\n", client.get_fd());

...
/*   Updated: 2023/04/12 13:45:16 by waelhamd         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
std::string server::join_response(std::vector<std::string> split, Client &client)
{
	...
	if(split.size() < 2)
		return (": "+client.get_nick()+" 461 JOIN :Not enough parameters\n");
	else if(split.size() >= 2)
	{
		...
		{
			if(names[i][0] != '#')
				return (": " + client.get_nick() + " 403 " + names[i] + " :No such channel\r\n");
			else if(client.check_member(names[i]))
				mysend(client.get_fd(), ": " + client.get_nick() + " 443 " + names[i] + " :is already on channel\r\n");
			else
			{
				...
				{//if channel exists
					...
					if(this->_channels[names[i]].get_inviteMode())
					{
						if(this->_channels[names[i]].is_invited(client.get_nick()))
						{
							...
							mysend(client.get_fd(), ":" + client.get_nick() + " 473 " + names[i] + " :Cannot join channel (+i)\r\n");
					}
					else
					{
						if(key.size() > i)
						{//if key is provided
							if(this->_channels[names[i]].get_pass() == key[i])
							{
								...
								mysend(client.get_fd(), ":" + client.get_nick() + " 475 " + names[i] + " :Cannot join channel (+k)\r\n");
						}
						else
		...
		return (": " + client.get_nick() + " 461 JOIN :Too many parameters\n");
